...
function MissionControl() {
  ...
  const [scheduleAgentFilter, setScheduleAgentFilter] = useState<
    AgentId | "all"
  >(() => {
  ...
  const [activityStatusFilter, setActivityStatusFilter] = useState<
    "all" | "running" | "completed" | "failed" | "pending" | "idle"
  >("all");
  ...
  const { activities, loading: activitiesLoading } = useAgentActivities(
    undefined,
    200,
  );
  ...
  const {
    ...
    loading: agentDocsLoading,
    error: agentDocsError,
  } = useAgentDocuments();
  const { activities: agentActivities, loading: agentActivitiesLoading } =
    useAgentActivities(undefined, 200);

  // Hooks for additional functionality
  const {
    isCompletedOnDate,
    toggleCompletion: toggleDateCompletion,
    getStatusOnDate,
    preloadCompletions,
  } = useTaskCompletions();
  ...
  // Overnight Summary
  const {
    activities: overnightActivities,
    stats: overnightStats,
    loading: overnightLoading,
  } = useOvernightSummary({ hours: 8 });
  ...
  const derivedStats = useMemo(() => {
    const map: Record<
      string,
      { tokens: number; tasks: number; activeSeconds: number }
    > = {};
  ...
  const timelineGroups = useMemo(() => {
    const groups = new Map<
      string,
      { ts: number; items: typeof agentActivities }
    >();
    for (const activity of agentActivities) {
      const ts = new Date(activity.timestamp).getTime();
      const label = new Date(activity.timestamp).toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      });
  ...
  const handleToggleTaskCompletion = async (task: Task, date: Date) => {
    const normalizedType =
      task.type || (task.day === "Daily" ? "daily" : "one-time");
    ...
    return tasks
      .map((task) => {
        ...
        if (isDaily) {
          return {
            ...task,
            status: normalizeTaskStatus(getStatusOnDate(task.id, date)),
          };
        }
  ...
  // Viewers cannot see the Documentation tab
  const tabs = isViewer
    ? allTabs.filter((t) => t.id !== "documentation")
    : allTabs;
      ...
      <div className="px-4 sm:px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            >
              {agentStates.filter((s) => s.isOnline).length}/
              {agentStates.length} Online
            </span>
          </motion.div>
          ...
          >
            ...
            <span
              className="text-xs font-medium"
              style={{
                color: isAdmin ? "var(--color-blue)" : "var(--color-green)",
              }}
            >
        ...
        return (
          ...
          >
            ...
            <span className="relative z-10 flex items-center gap-1.5">
              <Icon
                className={`w-3.5 h-3.5 ${!isActive ? "metallic-icon" : ""}`}
              />
              {tab.label}
              {tab.badge ? (
                <span
  ...
  const renderDashboard = () => {
    const totalTokens = agentStates.reduce(
      (sum, s) =>
        sum +
        (s.stats?.daily_tokens_used || derivedStats[s.agent.id]?.tokens || 0),
      0,
    );
    const totalTasks = agentStates.reduce(
      (sum, s) =>
        sum +
        (s.stats?.daily_tasks_completed ||
          derivedStats[s.agent.id]?.tasks ||
          0),
      0,
    );
    const totalActiveTime = agentStates.reduce(
      (sum, s) =>
        sum +
        (s.stats?.daily_active_seconds ||
          derivedStats[s.agent.id]?.activeSeconds ||
          0),
      0,
    );
    ...
    return (
      ...
      >
        <div className="mb-8">
          <h2
            className="text-2xl font-bold mb-1 text-gradient-metallic"
            style={{ letterSpacing: "-0.02em" }}
          >
            Dashboard
          </h2>
          <p className="text-sm" style={{ color: "var(--text-tertiary)" }}>
            Real-time statistics for all agents
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6 sm:mb-8">
          {[
            {
              label: "Active Agents",
              value: `${agentStates.filter((s) => s.isOnline).length}/${agentStates.length}`,
              icon: Users,
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    ...
                    >
                      ...
                      <p
                        ...
                        style={{ color: "var(--text-primary)" }}
                      >
                        {state.stats.daily_tokens_used.toLocaleString()}
                      </p>
                    </div>
                    <div
                      className="text-center rounded-xl py-2"
                    ...
                    >
                      ...
                      <p
                        ...
                        style={{ color: "var(--text-primary)" }}
                      >
                        {state.stats.daily_tasks_completed}
                      </p>
                    </div>
                  </div>
                )}
          ...
          <div className="apple-card p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                ...
                style={{ color: "var(--text-tertiary)" }}
              >
                {onlineAgents.length}/{agentStates.length} online
              </span>
            </div>
            {latest ? (
              <div
              ...
              >
                <p
                  ...
                  style={{ color: "var(--text-tertiary)" }}
                >
                  {AGENT_CONFIG[latest.agent_id as AgentId]?.name ||
                    latest.agent_id}
                </p>
          ...
          <div className="apple-card p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                ...
                style={{ color: "var(--text-tertiary)" }}
              >
                {agentSchedules.length} scheduled
              </span>
            </div>
            {upcoming.length ? (
  ...
  const renderActivityFeed = () => {
    ...
    const matchesFilters = (activity: (typeof agentActivities)[number]) => {
      if (
        ...
        activity.agent_id !== activityAgentFilter
      )
        return false;
      if (
      ...
      const haystack =
        `${activity.action} ${activity.description}`.toLowerCase();
    ...
    const actionCategory = (action: string) => {
      ...
      if (
        ["build", "building", "implementing", "coding"].some((k) =>
          normalized.includes(k),
        )
      )
        return { label: "Building", icon: Hammer };
      if (
        ["research", "researching", "analysis", "analyzing", "docs"].some((k) =>
          normalized.includes(k),
        )
      )
        return { label: "Researching", icon: Microscope };
      if (
        ["coordinating", "meeting", "sync", "syncing", "delegat"].some((k) =>
          normalized.includes(k),
        )
      )
        return { label: "Coordinating", icon: Users };
      if (["fix", "bug", "patch", "debug"].some((k) => normalized.includes(k)))
        return { label: "Fixing", icon: Bug };
    ...
    const ActivityColumn = ({ agentId }: { agentId: AgentId }) => {
      ...
      return (
        <div className="apple-card p-4 flex flex-col">
          ...
          {loading ? (
            <div
              className="flex items-center justify-center py-6 text-xs transition-colors duration-300"
              style={{ color: "var(--foreground)" }}
            >
              <motion.div
                animate={{ rotate: 360 }}
            ...
            <div className="space-y-2 max-h-[520px] overflow-y-auto pr-1">
              {filtered.map((activity) => {
                ...
                const StatusClass =
                  statusStyles[activity.status] || statusStyles.pending;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div
                        className="flex flex-wrap items-center gap-2 text-[10px] mt-1"
                        style={{ color: "var(--text-tertiary)" }}
                      >
    ...
    return (
      ...
      >
        <div className="mb-6">
          ...
          <p className="text-sm" style={{ color: "var(--text-tertiary)" }}>
            Newest on top · Last 50 per agent
          </p>
        </div>
        ...
        <div className="apple-card p-4 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-[1fr_180px_200px] gap-3 items-center">
            ...
            <select
              ...
              onChange={(e) =>
                setActivityStatusFilter(
                  e.target.value as typeof activityStatusFilter,
                )
              }
            >
              {["all", "running", "completed", "failed", "pending", "idle"].map(
                (status) => (
            ...
            <select
              ...
              onChange={(e) =>
                setActivityAgentFilter(e.target.value as AgentId | "all")
              }
            >
        ...
        {(() => {
          ...
          const filteredEventsCount = filteredTimelineGroups.reduce(
                      ...
                      {group.items.slice(0, 4).map((activity) => (
                        ...
                        >
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            >
                              {
                                AGENT_CONFIG[activity.agent_id as AgentId]
                                  ?.emoji
                              }
                            </span>
                            <span
  ...
  const renderCalendar = () => {
    ...
    const scheduleItems =
      scheduleAgentFilter === "all"
        ? agentSchedules
        : agentSchedules.filter(
            (schedule) => schedule.agent_id === scheduleAgentFilter,
          );
        ...
        <div className="mb-6 flex flex-col gap-4">
          <div>
            ...
            <p
              ...
              style={{ color: "var(--text-tertiary)" }}
            >
              {scheduleItems.length} items scheduled
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {(["begubot", "coder", "researcher", "extractor"] as AgentId[]).map(
              (agentId) => {
                const config = AGENT_CONFIG[agentId];
            ...
            {allTaskDays.map((date, idx) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <p
                      className="text-xs font-medium uppercase transition-colors duration-300"
                      style={{ color: "var(--text-tertiary)" }}
                    >
                  ...
                  <div className="p-3 space-y-2 min-h-[120px]">
                    {daySchedules.length > 0 ? (
                      daySchedules.map((schedule) => {
                        ...
                        const statusConfig = getScheduleStatusConfig(
                          schedule.status,
                        );
                        return (
                          <motion.div
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                ...
                                style={{ color: "var(--subtle)" }}
                              >
                                {formatScheduleTime(schedule.scheduled_for)}
                              </span>
                            </div>
                            {schedule.description && (
                              <p
                                className="mt-1 text-[10px] line-clamp-1 transition-colors duration-300"
                                style={{ color: "var(--subtle)" }}
                              >
                                {schedule.description}
                              </p>
                      ...
                      <p
                        className="text-xs text-center py-4 transition-colors duration-300"
                        style={{ color: "var(--subtle)" }}
                      >
                        No schedules
                      </p>
      ...
      <div className="mb-6 flex flex-col gap-4">
        <div>
          ...
          <p
            ...
            style={{ color: "var(--text-tertiary)" }}
          >
            Real-time agent documentation (workspace + memory)
          </p>
        </div>
      ...
      {agentDocsLoading ? (
        ...
        (() => {
          const filteredDocs = agentDocuments
            .filter(
              (doc) =>
                docsAgentFilter === "all" || doc.agent_id === docsAgentFilter,
            )
            .filter(
              (doc) =>
                !docsQuery ||
                doc.title.toLowerCase().includes(docsQuery.toLowerCase()) ||
                doc.content.toLowerCase().includes(docsQuery.toLowerCase()),
            );
          ...
          const buildAgentTree = (agentId: AgentId): FileNode => {
            ...
            const toFileNode = (doc: (typeof docs)[number]): FileNode => ({
              ...
              type: "file",
              isPriority: ["IDENTITY.md", "MEMORY.md", "SOUL.md"].includes(
                doc.title,
              ),
              content: doc.content,
            });
            ...
            const children: FileNode[] = Object.entries(categories)
              .sort(
                (a, b) =>
                  (categoryConfig[a[0]]?.order || 99) -
                  (categoryConfig[b[0]]?.order || 99),
              )
              .map(([cat, catDocs]) => {
          ...
          const tree: FileNode[] = [
            ...
            .filter(
              (agentId) =>
                docsAgentFilter === "all" || agentId === docsAgentFilter,
            )
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-4">
              <div className="apple-card p-3 max-h-[600px] overflow-y-auto">
                <FileTree
                  ...
                  onSelectFile={(file) =>
                    setSelectedDoc({ file, content: file.content || "" })
                  }
                />
              </div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p
            ...
            style={{ color: "var(--text-tertiary)" }}
          >
            Animated live office + real-time agent activity
          </p>
        </div>
      </div>
  ...
  const renderSearch = () => (
    ...
    >
      <div className="mb-8">
        <h2
          className="text-2xl font-bold mb-4 text-gradient-metallic"
          style={{ letterSpacing: "-0.022em" }}
        >
          Global Search
        </h2>
            ...
            >
              <div className="flex items-start justify-between gap-3 mb-2">
                <div className="flex-1">
                  ...
                  {doc.category && (
                    <span
                      className="inline-block mt-1 px-2 py-1 rounded-lg text-xs font-medium"
                      style={{
                        background: "var(--accent-muted)",
                        color: "var(--accent)",
                      }}
  ...
  return (
    ...
    >
      {/* Viewer overlay — block all pointer interactions on action elements */}
      {isViewer && (
        <style>{`
          .viewer-disabled { pointer-events: none !important; opacity: 0.5 !important; user-select: none !important; }
          ...
          <AnimatePresence mode="wait">
              ...
              >
                <div className="mb-6">
                  ...
                  <p
                    ...
                    style={{ color: "var(--text-tertiary)" }}
                  >
                    What got done today at a glance
                  </p>
                </div>
                <div className="grid grid-cols-1 xl:grid-cols-[2fr_1fr] gap-4">
                  <DailyReview />
                  <div className={isViewer ? "viewer-disabled" : ""}>
                    <WorkSessionTimer />
                  </div>
                </div>
              </motion.div>
            ...
            {activeTab === "workflows" && (
              ...
              >
                ...
                <div
                  className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 ${isViewer ? "viewer-disabled" : ""}`}
                >
                  <div className="space-y-6">
                    <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500 ml-1">
                      DevTools
                    </h3>
                    <SnippetGenerator />
                  </div>
              ...
              >
                <div className="mb-6">
                  ...
                  <p
                    ...
                    style={{ color: "var(--text-tertiary)" }}
                  >
                    Real-time health scores and workload balance
                  </p>
                </div>
                <div className="grid grid-cols-1 xl:grid-cols-[2fr_1fr] gap-4">
                  <AgentHealthCard />
                ...
                <div className="mt-8">
                  <h3 className="text-lg font-semibold mb-1 text-gradient-metallic">
                    System Health
                  </h3>
                  <p
                    ...
                    style={{ color: "var(--text-tertiary)" }}
                  >
                    Gateway monitoring and runtime visibility
                  </p>
                  <GatewayHealthCard />
                </div>
              </motion.div>
            ...
            {activeTab === "documentation" && isAdmin && renderDocumentation()}
      ...
      {/* Admin-only interactive tools */}
      {isAdmin && <QuickTaskCapture />}

      {/* Command Palette (⌘K) — admin only */}
      {isAdmin && (
        <CommandPalette
